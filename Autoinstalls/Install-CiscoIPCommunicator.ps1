function Install-CiscoIPCommunicator {
    <#
        .SYNOPSIS
    ...
    param (
        ...
        [String]$Name = "Cisco IP Communicator",
        ...
        [String[]]$ExcludedItems = @("cipc-Admin-fmr.8-6-4-0", "CiscoIPCommunicatorAdminToolSetup*", "L10n_Transforms"),

        [Parameter(Mandatory = $false,
            ValueFromPipeline = $true,
            ValueFromPipelineByPropertyName = $true)]
        [ValidateNotNullOrEmpty()]
        [string]$LogPath = "C:\Temp\CiscoIPCommunicator",
    ...
    process {
        ...
        try {
            ...
            $networkAdapter = (Get-NetAdapter | Where-Object { $_.name -like "Wi-Fi*" }).MacAddress -replace("-","")
 
            #######################################

            $latestSoftware = Get-ChildItem -path "$downloadPath\*.$extension" -Exclude $ExcludedItems -Recurse
            ...
            foreach ($Software in $latestSoftware) {
                ...
                $installerArgs = @{
                    ...
                    ArgumentList = @(
                        ...
                        'TRANSFORMS="1033.mst"',
                        'TFTP1="172.18.100.010"',
                        'TFTP2="172.18.120.010"'
                    )
                }
                ...
                if (!(test-path -Path "HKLM:\SOFTWARE\WOW6432Node\Cisco Systems, Inc.\Communicator")){
                    New-Item -Path "HKLM:\SOFTWARE\WOW6432Node\Cisco Systems, Inc." -Name "Communicator"
                }
                Set-ItemProperty -Path "HKLM:\SOFTWARE\WOW6432Node\Cisco Systems, Inc.\Communicator" -Name "HostName" -Value "SEP$networkAdapter"
                #######################################
...
Install-CiscoIPCommunicator -Path "\\Server\Path\Here\Settings\JSON\Applications.json"