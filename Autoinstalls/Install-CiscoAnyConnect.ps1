function Install-CiscoAnyConnect {
    ...
    param (
        ...
        [String]$Name = "Cisco AnyConnect",

        [Parameter(Mandatory = $false, ValueFromPipeline = $true)]
        [String]$Process = "vpnui",

        [Parameter(Mandatory = $false, ValueFromPipeline = $true)]
        [String]$Destination = "$home\Downloads",

        [Parameter(Mandatory = $false)]
        [String[]]$ExcludedItems = @("PKG","AnyConnect_Config Files","AnyconnectMac","InterfaceCleanup","OLD"),

        [Parameter(Mandatory = $false,
            ValueFromPipeline = $true,
            ValueFromPipelineByPropertyName = $true)]
        [ValidateNotNullOrEmpty()]
        [string]$LogPath = "C:\Temp\CiscoAnyConnect",
...
Install-CiscoAnyConnect -Path "\\Server\Path\Here\Settings\JSON\Applications.json"