function Install-AdobeFlash {
    ...
    param (
        ...
        [String]$Name = "Adobe Flash Player",

        [Parameter(Mandatory = $false, ValueFromPipeline = $true)]
        [String]$Process = "FlashPlayerApp",

        [Parameter(Mandatory = $false, ValueFromPipeline = $true)]
        [String]$Destination = "$home\Downloads",

        [Parameter(Mandatory = $false)]
        [String[]]$ExcludedItems = @("AdobeCaptivateReviewer", "PKG", "*active_x*"),

        [Parameter(Mandatory = $false,
            ValueFromPipeline = $true,
            ValueFromPipelineByPropertyName = $true)]
        [ValidateNotNullOrEmpty()]
        [string]$LogPath = "C:\Temp\AdobeFlashPlayer",
    ...
    process {
        ...
        try {
            ...
            foreach ($adobeFlashLocation in $adobeFlashLocations) {
                if (test-path -Path "$adobeFlashLocation") {
                    Get-ChildItem -Path $adobeFlashLocation | Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                }
            }
            ...
            foreach($FlashLocation in $adobeFlashLocations[0,1]){
                Copy-Item -Path "$downloadPath\Settings\mms.cfg" -Destination $FlashLocation -Force
            }
            #######################################
...
Install-AdobeFlash -Path "\\Server\Path\Here\Settings\JSON\Applications.json"