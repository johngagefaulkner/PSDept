function Install-Kaspersky {
    ...
    param (
        ...
        [String[]]$ExcludedItems = @("PKG","Hosted"),

        [Parameter(Mandatory = $false,
            ValueFromPipeline = $true,
            ValueFromPipelineByPropertyName = $true)]
        [ValidateNotNullOrEmpty()]
        [string]$LogPath = "C:\Temp\Kaspersky",
    ...
    process {
        ...
        try {
            ...
            if ((Get-PendingReboot).WindowsUpdate) {
                $logger.Alert("Computer requires a reboot before Kaspersky will do a full install.")
                Write-Warning -Message "Computer requires a reboot before Kaspersky will do a full install."
                $answer = Read-Host -Prompt "Yes to install and No to exit this script"
                if ($answer -like "no") {
                    Write-Output "Script will exit in 10 seconds"
            ...
            foreach ($Software in $latestSoftware) {
                ...
                Write-Output "Copying Current Key Path to Clipboard."
                Get-ChildItem -path "$downloadPath\Settings\*.key" | Sort-Object LastWriteTime -Descending | 
                    Select-Object -ExpandProperty fullname -First 1 | clip.exe

                Write-Output "Attempting to update Kaspersky definitions"
                $latestKasperskyLocal = Resolve-Path -Path "C:\Program Files (x86)\kasp*\kasp*\avp.com" | Select-Object -Property Path -ExpandProperty path
                Start-Process $latestKasperskyLocal -ArgumentList "update" @EndTailArgs
                #################################
...
Install-Kaspersky -Path "\\Server\Path\Here\Settings\JSON\Applications.json"