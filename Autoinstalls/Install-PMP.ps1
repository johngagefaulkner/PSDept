function Install-PMP {
    ...
    param (
        ...
        [String]$Name = "ManageEngine Patch Manager Plus",
        ...
        [String[]]$ExcludedItems = @("UEMSMacAgent*"),

        [Parameter(Mandatory = $false,
            ValueFromPipeline = $true,
            ValueFromPipelineByPropertyName = $true)]
        [ValidateNotNullOrEmpty()]
        [string]$LogPath = "C:\Temp\PMP",
    ...
    process {
        ...
        try {
            ...
            if (!($env:USERNAME -like "Non domainuserplace here")) {
                $searcher = [adsisearcher]"(samaccountname=$env:USERNAME)"
                $physicalDeliveryOfficeName = $searcher.FindOne().Properties.physicaldeliveryofficename
                $logger.informational("Physical Delivery Office: $physicalDeliveryOfficeName")

                if (!([string]::IsNullOrWhiteSpace($physicalDeliveryOfficeName))) {
                    if ($physicalDeliveryOfficeName -like "*offsite*") {
                        $alternate = $PMPSelection[1]
                    }
                    else {
                        $alternate = $PMPSelection[0]
                    }
                }
                else { 
                    $alternate = $PMPSelection[0]
                ...
                $alternate = Read-Host -Prompt " You are on on domainuserplace here. Please type either 'LocalOffice' or 'PMPDMZ'"
                if ($PMPSelection -notContains $alternate) {
                    $logger.warning("'$alternate' does not match LocalOffice or PMPDMZ. Exiting Script.")
                    Write-Warning "'$alternate' does not match LocalOffice or PMPDMZ. Exiting Script."
                    $logger.Remove()
                    Exit
                }
            }
            
            $logger.informational("Alternate Path: $alternate")
            #######################################
            ...
            $robocopyArgs = @{
                ...
                ArgumentList = @(
                    "\\$nearestDomain$($application.Program.$Name.Source)\$alternate"
                    "$Destination\$(Split-Path -Path $application.Program.$Name.Source -Leaf)\$alternate"
                    "/XD $ExcludedItems"
                    "/E"
                    "/R:5"
                    "/W:5"
            ...
            $latestSoftware = Get-ChildItem -path "$downloadPath\$alternate\*.$extension" -Exclude $ExcludedItems -Recurse
            ...
            $ruleChanges = (Get-NetFirewallRule -DisplayGroup 'Network Discovery' | Set-NetFirewallRule -Profile 'Private, Domain' -Enabled true -PassThru)
            $ruleChanges2 = (Get-NetFirewallRule -DisplayGroup 'File and Printer Sharing' | Set-NetFirewallRule -Profile 'Private, Domain' -Enabled true -PassThru)
            ...
            foreach ($change in $Changes) {
                foreach ($c in $change) {
                    if ($c.status -notlike "The rule was parsed successfully from the store. (65536)") {
                        $logger.warning("A rule was not successful")
                        $badRuleChange = [PSCustomObject]@{
                            Name          = $c.Name
...
Install-PMP -Path "\\Server\Path\Here\Settings\JSON\Applications.json"