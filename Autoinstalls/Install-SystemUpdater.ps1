function Install-SystemUpdater {
    <#
        .SYNOPSIS
        ...
        .LINK
            ...
            https://www.dell.com/support/manuals/us/en/04/command-update-v3.1/dellcommandupdate_3.1_ug/command-line-interface-reference?guid=guid-92619086-5f7c-4a05-bce2-0d560c15e8ed&lang=en-us
            --------------
    ...
    param (
        ...
        [String]$Name = "Dell Command | Update",
        ...
        [String[]]$ExcludedItems = @("HPSupportAssistant*"),

        [Parameter(Mandatory = $false,
            ValueFromPipeline = $true,
            ValueFromPipelineByPropertyName = $true)]
        [ValidateNotNullOrEmpty()]
        [string]$LogPath = "C:\Temp\SystemUpdater",
    ...
    process {
        ...
        try {
           ...
           if ($Manufacturer -like "*Lenovo*") {
               ...
               $whereFilter = "*Dell-Command-Update*"
               $Name = "Dell Command | Update"
           } 
            <#elseif ($Manufacturer -like "*Hewlett*") {
                $whereFilter = "*HP*"
                $Name = ""
            }#>
            #######################################

            $latestSoftware = Get-ChildItem -path "$downloadPath\$extension\*.$extension" -Exclude $ExcludedItems -Recurse | Where-Object {$_.Name -like $whereFilter}
            ...
            foreach ($Software in $latestSoftware) {
                ...
                switch -Wildcard ($Manufacturer) {
                    "*Lenovo*" {     
                        Regedit /s "\\Server\Path\Here\Settings\REG\LenovoSU.reg"
                        Start-Process "C:\Program Files (x86)\Lenovo\System Update\tvsuShim.exe" -ArgumentList "/CM -search R -action INSTALL -includerebootpackages 1,3,4 -noicon -noreboot -nolicense -defaultupdate"
                        Break
                    }
                    "*Dell*" {
                        Start-Process -filepath "C:\Program Files (x86)\Dell\CommandUpdate\dcu-cli.exe" -ArgumentList "-importSettings=\\Server\Path\Here\Settings\XML\DellCommandUpdateSettings.xml"       
                        Start-Process -filepath "C:\Program Files (x86)\Dell\CommandUpdate\dcu-cli.exe" -ArgumentList "/scan"
                        Start-Process -filepath "C:\Program Files (x86)\Dell\CommandUpdate\dcu-cli.exe" -ArgumentList "/applyupdates"
                        Break
                    }
                    Default {"System is not supported for scanning and updating."}
                }
                #######################################
...
Install-SystemUpdater -Path "\\Server\Path\Here\Settings\JSON\Applications.json"