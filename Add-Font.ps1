function Add-Font {
    ...
    .EXAMPLE
        Add-Font -path "C:\Users\user\Downloads\"

    .EXAMPLE
        Add-Font -path "C:\Users\user\Downloads\Cascadia.ttf"
    ...
    param (
        ...
        [System.IO.FileInfo]$Path = "\\Server\Path\Here\Fonts",
    ...
    begin {
        ...
        $logger.informational("Building current installed font list")
...
'@
        ...
        function Add-SingleFont($InputObject) {
            try {
                [string]$filePath = (resolve-path $InputObject).path
                [string]$fileDir = split-path $InputObject
                [string]$fileName = split-path $InputObject -leaf
                [string]$fileExt = (Get-Item $InputObject).extension
                ...
                if ($PSCmdlet.ShouldProcess("$filePath",'Copy Item')){
                ...
                if ($PSCmdlet.ShouldProcess("$fontFinalPath",'Add Font')){
                    ...
                    if ($retVal -eq 0) {
                        ...
                        $logger.Error("Font $filePath installation failed")
                        return $False
                    }
                    else {
                        if ($PSCmdlet.ShouldProcess("Item: $FontRegistryPath Property: $($fontName)$($hashFontFileTypes.item($fileExt))",'Set Property')){
                ...
                $erroredFonts.Add("$($error[0].ToString())")
                $error.clear()
                return $False
            }
        }
    ...
    process { 
        ...
        $output = [PSCustomObject]@{
            ...
            SuccessfullyInstalledFonts = $SuccessfullyInstalledFonts
