function Install-SoftwareTemplate {
    <#
        .SYNOPSIS
        ...
        .LINK
            https://git.renaissance.com/HelpDesk/Auto_Installs/blob/master/README.md
        .NOTES
            None
    #>
    [CmdletBinding()]
    ...
    begin {
        ...
        try {
            if (!("PSLogger" -as [type])) {
                ...
                ."\\wrpwi\helpdesk\Windows10Fixes\Settings\Scripts\Logging.ps1"
        ...
        try {
            ...
            function Get-NearestDomain {
                ...
                process {
                    ...
                    # Switches nearest domain to wrpwi from a WR or HQ named DC
                    if ($nearestDomain -like "DC**WR" -or $nearestDomain -like "DC**HQ") {
                        $nearestDomain = "WRPWI"
                    }
                }
            ...
            if (!(Get-Variable -Name nearestDomain -ErrorAction SilentlyContinue)) {
                $nearestDomain = (Get-NearestDomain -Path "\\wrpwi\helpdesk\Windows10Fixes\Settings\JSON\DCList.json")
            }
        }
        catch {
            $logger.Error("$PSitem")
...
#Install- -Path "\\wrpwi\helpdesk\Windows10Fixes\Settings\JSON\Applications.json"